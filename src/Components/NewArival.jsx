import React from "react";

const NewArival = () => {
  return (
      <div className="max-w-7xl mx-auto px-4 py-10">

       <p className="text-red-500 font-semibold flex items-center px-5 mb-2">
        <span className="w-3 h-6 bg-red-500 mr-2 rounded-sm"></span> Featured
      </p>
      {/* Title */}
      <h2 className="text-3xl font-bold my-6 px-5">
        New Arrival</h2>
    <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-2 gap-6">

      {/* Left Section */}
      <div className="bg-black rounded-xl overflow-hidden flex items-end justify-start relative">
        <img
          src="/images/Playstation.png"
          alt="PlayStation 5"
          className="w-full h-full object-cover"
        />
        <div className="absolute bottom-4 left-4 text-white text-xl font-semibold">
          PlayStation 5
        </div>
      </div>

      {/* Right Section */}
      <div className="flex flex-col gap-6">
        
        {/* Top */}
        <div className="bg-black text-white rounded-xl overflow-hidden relative p-6 flex flex-col justify-end min-h-[200px] md:min-h-[250px]">
          <img
            src="/images/girl.jpg"
            alt="Women's Collection"
            className="absolute inset-0 w-full h-full object-cover opacity-40"
          />
          <div className="relative z-10">
            <h3 className="text-lg md:text-xl font-semibold mb-1">
              Women’s Collections
            </h3>
            <p className="text-sm mb-2">
              Featured woman collections that give you another vibe.
            </p>
            <button className="underline text-sm font-medium">
              Shop Now
            </button>
          </div>
        </div>

        {/* Bottom */}
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-black rounded-xl overflow-hidden">
            <img
              src="/images/WirlessSpeaker.png"
              alt="Alexa"
              className="w-full h-full object-cover"
            />
          </div>
          <div className="bg-black rounded-xl overflow-hidden">
            <img
              src="/images/Perfume.png"
              alt="Gucci Perfume"
              className="w-full h-full object-cover"
            />
          </div>
        </div>

      </div>
    </div>
    </div>
  );
};

export default NewArival;
